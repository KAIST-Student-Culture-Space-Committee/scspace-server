...
export class ArticleService {
  ...
  async createArticle(
    userId: number,
    articleData: Omit<IArticleCreate, 'userId'>,
  ): Promise<IArticle> {
  ...
  async getArticlePreviews(): Promise<IArticlePreviewWithUser> {
    const notice = await this.articleRepository.getArticlePreviewByType(
      ArticleTypeEnum.NOTICE,
    );
    const business = await this.articleRepository.getArticlePreviewByType(
      ArticleTypeEnum.BUSINESS,
    );
    const promotion = await this.articleRepository.getArticlePreviewByType(
      ArticleTypeEnum.PROMOTION,
    );
  ...
  async getUserArticles(
    userId: number,
    query: Omit<IArticleQuery, 'userId'> = {},
  ): Promise<{
    articles: IArticle[];
    total: number;
    page: number;
    ...
    const result = await this.articleRepository.getArticlesByUserId(
      userId,
      query,
    );
    const totalPages = Math.ceil(result.total / limit);
  ...
  async searchArticles(
    searchTerm: string,
    query: Omit<IArticleQuery, 'search'> = {},
  ): Promise<{
    articles: IArticleWithUser[];
    total: number;
    page: number;
